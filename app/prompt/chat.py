...
def respond(user_msg, chat_hist):
    ...
    chat_sys_message_prompt = SystemMessagePromptTemplate.from_template(chat_sys_template)
    chat_human_message_prompt = HumanMessagePromptTemplate.from_template(chat_user_template)
    chat_messages = ChatPromptTemplate.from_messages([chat_sys_message_prompt, chat_human_message_prompt])
    ...
    chat_prompt = chat_messages.format_prompt(scenario=scenario, chat_history = chat_history, user_answer = user_msg)
    chat_prompt_str = chat_prompt.to_string()
    
    question = chatbot(chat_prompt_str)
    chat_hist += [(user_msg, question)]
    
    return  question, '', chat_hist, chat_prompt_str
...
def main():
    with gr.Blocks() as demo:
        ...
        user_msg.submit(respond, [user_msg, chat_history], [question, user_msg, chatbot, chat_prompt])
